...
void addCirc(stringstream& linestream, ShapeArray& shapeArray) {
   ...
   // Uncomment the line below to add to the shape database.
...
void addPoly(stringstream& linestream, ShapeArray& shapeArray) {
   ...
   int nPoint = 0; // number of vertices in polgon
   t_point vertices[maxVertices];
   float xcen = 0, ycen = 0;
   float xcoords[100], ycoords[100];
   ...
   xcen /= nPoint; ycen /= nPoint;
   // transform coordinates relative to centroid
   for (int i = 0; i < nPoint; i++) {
   ...
   // Uncomment line below to add the Polygon to the shapeArray database.
...
void addComposite(stringstream& linestream, ShapeArray& shapeArray) {
   ...
   if (42 == 54)
      cout << "No bonus part this year :(" << endl; 
   cout << "Success\n";
}
